...
export class SonicStudioShell {
    constructor(choreographer, { isMobile = false } = {}) {
        ...
        this.storyboardEntries = [];
        this.responsivePresetEntries = [];
    ...
    cacheElements() {
        ...
        this.elements.storyboardList = document.getElementById('mood-storyboard-list');
        this.elements.storyboardEmpty = document.getElementById('mood-storyboard-empty');
        this.elements.presetList = document.getElementById('responsive-preset-list');
        this.elements.presetEmpty = document.getElementById('responsive-preset-empty');
        this.elements.presetTrain = document.querySelector('[data-action="train-responsive-preset"]');
    }
    ...
    installEventHandlers() {
        ...
        if (this.elements.presetTrain) {
            this.elements.presetTrain.addEventListener('click', () => this.handleTrainResponsivePreset());
        }

        if (this.elements.presetList) {
            this.elements.presetList.addEventListener('click', (event) => this.handlePresetListClick(event));
        }
    }
    ...
    async initializeMoodConductor() {
        ...
        try {
            ...
            this.moodConductor.onStoryboardChange((storyboard) => this.renderMoodStoryboard(storyboard));
            this.moodConductor.onPresetChange((presets, latest, activeId) => this.renderResponsivePresets(presets, latest, activeId));
            ...
            this.renderMoodStoryboard(this.moodConductor.getStoryboard());
            const presets = this.moodConductor.getResponsivePresets();
            const activePreset = this.moodConductor.getActiveResponsivePreset();
            this.renderResponsivePresets(presets, null, activePreset?.id || null);
        ...
        const html = recent.map(entry => {
            const typeLabel = (() => {
                switch (entry.type) {
                    case 'ai-profile':
                        return 'AI Mood';
                    case 'quick-adjust-reset':
                        return 'Reset';
                    case 'mood-recall':
                        return 'Recall';
                    case 'responsive-preset':
                        return entry.auto ? 'Auto Preset' : 'Preset';
                    case 'responsive-preset-apply':
                        return 'Preset Apply';
                    default:
                        return 'Adjust';
                }
            ...
            const presetIdAttr = entry.preset?.id
                ? ` data-preset-id="${this.escapeHtml(entry.preset.id)}"`
                : '';
            return `
                <li class="mood-history-item" data-entry-id="${this.escapeHtml(entry.id || '')}" data-entry-type="${this.escapeHtml(entry.type || 'ai-profile')}"${presetIdAttr}${entry.referenceId ? ` data-reference-id="${this.escapeHtml(entry.referenceId)}"` : ''}>
                    <header>
    ...
    renderMoodStoryboard(storyboard = []) {
        this.storyboardEntries = Array.isArray(storyboard) ? storyboard : [];
        const list = this.elements.storyboardList;
        const emptyState = this.elements.storyboardEmpty;
        if (!list) return;

        if (!this.storyboardEntries.length) {
            list.innerHTML = '';
        ...
        const html = this.storyboardEntries.map((entry, index) => {
            const isActive = index === this.storyboardEntries.length - 1;
            const start = Number.isFinite(entry.startTime) ? this.formatTime(entry.startTime) : '--:--';
            const duration = Number.isFinite(entry.duration) ? `${entry.duration.toFixed(1)}s` : '∞';
            const energy = Number.isFinite(entry.energy) ? `${Math.round(entry.energy * 100)}%` : '—';
            const keywords = Array.isArray(entry.keywords) && entry.keywords.length
                ? entry.keywords.slice(0, 3).map(word => this.escapeHtml(word)).join(', ')
                : '—';
            return `
                <li class="mood-storyboard-item${isActive ? ' is-active' : ''}" data-storyboard-id="${this.escapeHtml(entry.id || '')}">
                    <header>
                        <span class="storyboard-label">${this.escapeHtml(entry.label || 'Mood')}</span>
                        <span class="storyboard-time">${start} · ${duration}</span>
                    </header>
                    <div class="storyboard-energy">Energy ${this.escapeHtml(energy)}</div>
                    <div class="storyboard-keywords">${keywords}</div>
                </li>
            `;
        }).join('');
    ...
    renderResponsivePresets(presets = [], latest = null, activeId = null) {
        this.responsivePresetEntries = Array.isArray(presets) ? presets : [];
        const list = this.elements.presetList;
        const emptyState = this.elements.presetEmpty;
        if (!list) return;

        if (!this.responsivePresetEntries.length) {
            list.innerHTML = '';
            if (emptyState) emptyState.hidden = false;
            return;
        }
        ...
        const html = this.responsivePresetEntries.map(preset => {
            const weights = preset.weights || {};
            const weightText = (weights.mood !== undefined || weights.pad !== undefined || weights.keyboard !== undefined)
                ? `M${Math.round((weights.mood ?? 0) * 100)} · P${Math.round((weights.pad ?? 0) * 100)} · K${Math.round((weights.keyboard ?? 0) * 100)}`
                : '';
            const timestamp = preset.createdAt ? this.formatTimestampShort(preset.createdAt) : '';
            const isActive = preset.id === activeId;
            return `
                <li class="responsive-preset-item${isActive ? ' is-active' : ''}" data-preset-id="${this.escapeHtml(preset.id || '')}">
                    <span class="preset-label">${this.escapeHtml(preset.label || 'Responsive')}</span>
                    <span class="preset-weights">${this.escapeHtml(weightText)}</span>
                    <time>${this.escapeHtml(timestamp)}</time>
                </li>
            `;
        }).join('');
    ...
    handleMoodJournalClick(event) {
        ...
        const { entryId, entryType, presetId } = item.dataset;

        if (entryType && entryType.startsWith('responsive-preset') && presetId) {
            this.moodConductor.applyResponsivePreset(presetId, { record: true });
    ...
    handleTrainResponsivePreset() {
        if (!this.moodConductor) {
            this.initializeMoodConductor().then(() => {
                this.handleTrainResponsivePreset();
            });
            return;
        }
        const context = this.moodConductor.buildPromptContext?.() || null;
        this.moodConductor.trainResponsivePreset({ context, auto: false, apply: true });
    }

    handlePresetListClick(event) {
        const item = event.target.closest('.responsive-preset-item');
        if (!item || !item.dataset.presetId) {
            return;
        }

        if (!this.moodConductor) {
            this.initializeMoodConductor().then(() => {
                this.moodConductor?.applyResponsivePreset(item.dataset.presetId, { record: true });
            });
            return;
        }
