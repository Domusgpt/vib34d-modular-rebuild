...
export class SonicControlMatrix {
    ...
    apply(audioData, choreographyTargets = {}) {
        ...
        const bpm = this.choreographer?.audioAnalyzer?.getBPM?.() || 0;
        const spectralBalance = this.computeSpectralBalance(audioData);
        const genre = this.estimateGenre(audioData, bpm);
        ...
        const frameInfo = {
            audio: {
                ...
                context: {
                    bpm,
                    spectralBalance,
                    spectralSignature: spectralBalance.signature,
    ...
    computeSpectralBalance(audioData = {}) {
        const bass = Math.max(0, audioData.bass ?? audioData.momentum?.bass ?? 0);
        const mid = Math.max(0, audioData.mid ?? audioData.lowMid ?? 0);
        const high = Math.max(0, audioData.high ?? audioData.highMid ?? 0);
        ...
        normalized.signature = this.describeSpectralSignature(normalized);
        return normalized;
    }

    describeSpectralSignature(balance = {}) {
        const { bass = 0, mid = 0, high = 0 } = balance;
        if (bass > 0.48 && bass - Math.max(mid, high) > 0.12) {
            return 'bass-forward';
        }
        if (mid > 0.42 && mid - Math.max(bass, high) > 0.1) {
            return 'mid-rich';
        }
        if (high > 0.4 && high - Math.max(bass, mid) > 0.1) {
            return 'treble-bright';
        }
        if (bass > 0.38 && high > 0.32) {
            return 'wide-spectrum';
        }
        return 'balanced';
    }

    estimateGenre(audioData = {}, bpm = 0) {
        const energy = audioData.energy ?? 0;
        const balance = this.computeSpectralBalance(audioData);
        const { bass, mid, high } = balance;

        if (bpm > 135 && energy > 0.7 && bass > 0.45 && high > 0.35) {
            return 'drum-and-bass';
        }
        if (bpm >= 120 && energy > 0.6 && high > bass && high > mid) {
            return 'electro';
        }
        if (bpm >= 118 && bass > 0.5 && energy > 0.55) {
            return 'club-house';
        }
        if (bpm <= 95 && mid > 0.4 && energy < 0.55) {
            return 'lofi';
        }
        if (energy < 0.35 && bass < 0.3 && high < 0.35) {
            return 'ambient';
        }
        if (mid > 0.36 && high > 0.33 && energy > 0.5) {
            return 'cinematic';
        }
        return 'hybrid';
    }
