...
export class SonicControlMatrix {
    ...
    captureResponsivePreset(label, { moodProfile, weights = {}, telemetry = null, surfaces = null, keyboard = null } = {}) {
        const id = `preset-${Date.now().toString(36)}-${Math.random().toString(16).slice(2, 8)}`;
        const baseParameters = this.getBaseParameters();
        const moodParameters = moodProfile?.parameters || baseParameters;
        const padOffsets = this.padOffsets || {};
        const keyboardOffsets = this.keyboardOffsets || {};

        const weightMood = weights.mood ?? 0.7;
        const weightPad = weights.pad ?? 0.2;
        const weightKeyboard = weights.keyboard ?? 0.1;
        const weightTotal = Math.max(weightMood + weightPad + weightKeyboard, 0.0001);
        const normalizedWeights = {
            mood: weightMood / weightTotal,
            pad: weightPad / weightTotal,
            keyboard: weightKeyboard / weightTotal
        };

        const parameters = {};
        Object.keys(this.parameterDefinitions).forEach(param => {
            const def = this.getParameterDefinition(param);
            const moodValue = moodParameters[param] ?? baseParameters[param] ?? def.default ?? 0;
            const padValue = moodValue + (padOffsets[param] || 0);
            const keyboardValue = moodValue + (keyboardOffsets[param] || 0);
            const blended = (moodValue * normalizedWeights.mood)
                + (padValue * normalizedWeights.pad)
                + (keyboardValue * normalizedWeights.keyboard);
            parameters[param] = clamp(blended, def.min, def.max);
        });
        ...
        const preset = {
            ...
            moodLabel: moodProfile?.mood?.label || null,
            context: {
                telemetry,
                surfaces: surfaces || this.getSurfaceSnapshot(),
                keyboard: keyboard || { ...keyboardOffsets }
            }
        };

        this.responsivePresets.set(id, preset);
        return preset;
    }

    applyResponsivePreset(presetId, { immediate = true } = {}) {
        const preset = typeof presetId === 'string'
            ? this.responsivePresets.get(presetId)
            : presetId;
        ...
        const batch = this.applyParameterBatch(preset.parameters, { immediate });
        return {
            ...preset,
    ...
    getResponsivePreset(presetId) {
        return this.responsivePresets.get(presetId) || null;
    }

    listResponsivePresets() {
        return Array.from(this.responsivePresets.values());
    }
}
