...
Blend mood, color, and motion so the visuals feel emotionally true to the description.`;
    ...
    async generateParameters(description) {
        ...
        const rawProfile = await this.fetchRawProfile(trimmed);
    ...
    async fetchRawProfile(description) {
        let lastError = null;
        ...
        try {
            return await this.generateViaDirect(description);
        } catch (error) {
            if (lastError) {
                throw error;
            }
    ...
    async generateViaDirect(description) {
