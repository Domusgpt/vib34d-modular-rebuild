...
export class SonicMoodConductor {
    constructor(choreographer, options = {}) {
        ...
        this.performanceArcListeners = new Set();
        this.performanceArcs = [];
        this.performanceArcState = this.createEmptyArcState();
        this.autoPerformanceArcId = 'storyboard-orbit';
        this.performanceArcProgressIntervalMs = options.performanceArcProgressIntervalMs || 240;
    ...
    onPerformanceArcUpdate(listener) {
        if (typeof listener === 'function') {
            this.performanceArcListeners.add(listener);
        }
    }

    offPerformanceArcUpdate(listener) {
        if (listener && this.performanceArcListeners.has(listener)) {
            this.performanceArcListeners.delete(listener);
        }
    }

    notifyPerformanceArcListeners() {
        const arcs = this.getPerformanceArcs();
        const state = this.getPerformanceArcState();
        this.performanceArcListeners.forEach(listener => {
            try {
                listener(arcs, state);
            } catch (error) {
                console.error('SonicMoodConductor performance arc listener error:', error);
            }
        });
    }

    getPerformanceArcs() {
        return this.performanceArcs.map(arc => this.cloneArc(arc));
    }

    getPerformanceArcState() {
        const state = this.performanceArcState || this.createEmptyArcState();
        if (!state.activeArcId) {
            return { ...this.createEmptyArcState() };
        }

        const arc = this.getPerformanceArcById(state.activeArcId);
        if (!arc || !arc.stages || !arc.stages.length) {
            return { ...this.createEmptyArcState() };
        }

        const stage = arc.stages[state.stageIndex] || arc.stages[0];
        const stageDuration = stage?.duration && stage.duration > 0 ? stage.duration : 1;
        const stageProgress = clamp(state.stageElapsed / stageDuration, 0, 1);
        const totalDuration = arc.totalDuration ?? this.getArcTotalDuration(arc);
        const effectiveTotal = totalDuration > 0 ? totalDuration : 1;
        const overallElapsed = arc.loop
            ? state.totalElapsed % effectiveTotal
            : Math.min(state.totalElapsed, effectiveTotal);
        const overallProgress = clamp(overallElapsed / effectiveTotal, 0, 1);

        return {
            activeArcId: state.activeArcId,
            stageIndex: Math.min(state.stageIndex, arc.stages.length - 1),
            stageProgress,
    ...
    getPerformanceArcById(arcId) {
        if (!arcId) {
            return null;
        }
        return this.performanceArcs.find(arc => arc.id === arcId) || null;
    }
    ...
    cloneArc(arc) {
        ...
        return {
            ...
            stages: (arc.stages || []).map(stage => ({
                ...stage,
                profileSnapshot: stage.profileSnapshot ? this.cloneForLog(stage.profileSnapshot) : null
            }))
        };
    }

    getArcTotalDuration(arc) {
    ...
    computeArcTotals(arc) {
        if (!arc) {
            return;
        }
        arc.stageCount = (arc.stages || []).length;
        arc.totalDuration = this.getArcTotalDuration(arc);
    }

    normalizeArcStage(stageConfig = {}, index = 0) {
        const baseEntry = stageConfig.storyboardEntryId && this.storyboard
            ? this.storyboard.getEntry(stageConfig.storyboardEntryId)
            : null;
        const snapshotSource = stageConfig.profileSnapshot
            || baseEntry?.profileSnapshot
            || null;
        const snapshot = snapshotSource ? this.cloneForLog(snapshotSource) : null;
        const durationSource = stageConfig.duration ?? baseEntry?.duration ?? 12;
        const duration = Math.max(0.5, Number.isFinite(durationSource) ? durationSource : 12);
        const label = stageConfig.label || baseEntry?.label || `Stage ${index + 1}`;
        const prompt = stageConfig.prompt || baseEntry?.prompt || snapshot?.mood?.description || '';
        return {
            id: stageConfig.id || `arc-stage-${index}-${Date.now().toString(36)}-${Math.random().toString(16).slice(2, 6)}`,
            label,
            prompt,
            profileId: stageConfig.profileId || baseEntry?.profileId || stageConfig.storyboardEntryId || null,
            storyboardEntryId: stageConfig.storyboardEntryId || baseEntry?.id || null,
            duration,
            transition: stageConfig.transition || 'cut',
            manualInfluence: clamp(stageConfig.manualInfluence ?? baseEntry?.manualInfluence ?? 0, 0, 1),
            profileSnapshot: snapshot,
            order: index
        };
    }

    areStagesEqual(existing = [], next = []) {
        ...
        for (let i = 0; i < existing.length; i += 1) {
            const prevStage = existing[i];
            const nextStage = next[i];
            if (!prevStage || !nextStage) {
                return false;
            }
            const durationDelta = Math.abs((prevStage.duration || 0) - (nextStage.duration || 0));
            if (
                prevStage.storyboardEntryId !== nextStage.storyboardEntryId
                || prevStage.profileId !== nextStage.profileId
                || prevStage.label !== nextStage.label
                || durationDelta > 1e-3
            ) {
                return false;
            }
    ...
    syncAutoPerformanceArc() {
        const entries = this.storyboard ? this.storyboard.getEntries() : [];
        const autoId = this.autoPerformanceArcId;
        const existingIndex = this.performanceArcs.findIndex(arc => arc.id === autoId);

        if (!entries || entries.length === 0) {
            if (existingIndex !== -1) {
                if (this.performanceArcState.activeArcId === autoId) {
                    this.deactivatePerformanceArc({ reason: 'auto-removed', log: false });
                }
                this.performanceArcs.splice(existingIndex, 1);
                this.notifyPerformanceArcListeners();
            }
            return;
        }

        const stages = entries.map((entry, index) => this.normalizeArcStage({
            id: entry.id,
            label: entry.label,
            prompt: entry.prompt,
        ...
        if (existingIndex !== -1) {
            const arc = this.performanceArcs[existingIndex];
            if (!this.areStagesEqual(arc.stages, stages)) {
                arc.stages = stages;
                updated = true;
            }
            const label = 'Storyboard Orbit';
            const description = 'Autoplay the storyboard moods as a continuous journey.';
            if (arc.label !== label) {
                arc.label = label;
                updated = true;
            }
            if (arc.description !== description) {
            ...
            if (!arc.tempoScale || arc.tempoScale <= 0) {
                arc.tempoScale = 1;
                updated = true;
            }
            arc.metadata = { ...(arc.metadata || {}), autoGenerated: true };
            this.computeArcTotals(arc);

            if (this.performanceArcState.activeArcId === autoId) {
                if (!arc.stages.length) {
                    this.deactivatePerformanceArc({ reason: 'auto-removed', log: false });
                } else if (this.performanceArcState.stageIndex >= arc.stages.length) {
                    this.performanceArcState.stageIndex = Math.max(0, arc.stages.length - 1);
                    this.performanceArcState.stageElapsed = Math.min(
                        this.performanceArcState.stageElapsed,
                        arc.stages[this.performanceArcState.stageIndex].duration
                    );
                }
            }
        } else {
            const arc = {
                id: autoId,
                label: 'Storyboard Orbit',
                description: 'Autoplay the storyboard moods as a continuous journey.',
                stages,
                loop: true,
                tempoScale: 1,
                createdAt: Date.now(),
    ...
    activatePerformanceArc(arcId, { restart = true, silent = false } = {}) {
        const arc = this.getPerformanceArcById(arcId);
        if (!arc || !arc.stages || !arc.stages.length) {
            return null;
        }

        if (!restart && this.performanceArcState.activeArcId === arcId) {
            return this.getPerformanceArcState();
        }

        const now = this.getNow();
        this.performanceArcState = {
            ...this.createEmptyArcState(),
            activeArcId: arc.id,
            stageIndex: 0,
            lastTick: now,
        ...
        if (!silent) {
            this.recordSessionEntry('performance-arc-activate', {
                arcId: arc.id,
                arcLabel: arc.label,
                stageCount: arc.stages.length,
                loop: arc.loop,
                tempoScale: arc.tempoScale ?? 1
            });
        }

        const firstStage = arc.stages[0];
        if (firstStage) {
            this.applyArcStage(arc, firstStage);
        }
    ...
    deactivatePerformanceArc({ reason = 'manual-stop', log = true } = {}) {
        if (!this.performanceArcState.activeArcId) {
            return null;
        }

        const arc = this.getPerformanceArcById(this.performanceArcState.activeArcId);
        const info = {
            arcId: this.performanceArcState.activeArcId,
            arcLabel: arc?.label || 'Performance Arc',
            stageIndex: this.performanceArcState.stageIndex,
            loopCount: this.performanceArcState.loopCount,
            reason
        };
        ...
        if (log) {
            const type = reason === 'complete' ? 'performance-arc-complete' : 'performance-arc-stop';
            this.recordSessionEntry(type, info);
        }
    ...
    advancePerformanceArc(timestamp) {
        if (!this.performanceArcState.activeArcId) {
            return null;
        }
        ...
        if (this.performanceArcState.lastTick === null) {
            this.performanceArcState.lastTick = now;
            return this.getPerformanceArcState();
        }

        const deltaMs = now - this.performanceArcState.lastTick;
        ...
        const scaledDelta = deltaSeconds * tempoScale;

        this.performanceArcState.stageElapsed += scaledDelta;
        this.performanceArcState.totalElapsed += scaledDelta;
        this.performanceArcState.lastTick = now;

        const stage = arc.stages[this.performanceArcState.stageIndex];
        if (!stage) {
            this.deactivatePerformanceArc({ reason: 'auto-removed', log: false });
            return null;
        }
        ...
        const stageDuration = stage.duration && stage.duration > 0 ? stage.duration : 1;
        this.performanceArcState.stageProgress = clamp(this.performanceArcState.stageElapsed / stageDuration, 0, 1);

        const totalDuration = arc.totalDuration ?? this.getArcTotalDuration(arc);
        const effectiveTotal = totalDuration > 0 ? totalDuration : 1;
        const overallElapsed = arc.loop
            ? this.performanceArcState.totalElapsed % effectiveTotal
            : Math.min(this.performanceArcState.totalElapsed, effectiveTotal);
        this.performanceArcState.overallProgress = clamp(overallElapsed / effectiveTotal, 0, 1);

        if (this.performanceArcState.stageElapsed >= stage.duration - 1e-3) {
            this.advanceArcStage(arc, now);
            return this.getPerformanceArcState();
        }

        if (!this.performanceArcState.lastNotifiedAt
            || now - this.performanceArcState.lastNotifiedAt >= this.performanceArcProgressIntervalMs) {
            this.performanceArcState.lastNotifiedAt = now;
            this.notifyPerformanceArcListeners();
        }
    ...
    advanceArcStage(arc, now) {
        if (!arc || !arc.stages || !arc.stages.length) {
            this.deactivatePerformanceArc({ reason: 'auto-removed', log: false });
            return;
        }

        let nextIndex = this.performanceArcState.stageIndex + 1;
        if (nextIndex >= arc.stages.length) {
            if (arc.loop) {
                this.performanceArcState.loopCount += 1;
                this.recordSessionEntry('performance-arc-loop', {
                    arcId: arc.id,
                    arcLabel: arc.label,
                    loopCount: this.performanceArcState.loopCount
                });
                ...
                this.deactivatePerformanceArc({ reason: 'complete', log: true });
                return;
            }
        }

        this.performanceArcState.stageIndex = nextIndex;
        this.performanceArcState.stageElapsed = 0;
        this.performanceArcState.stageProgress = 0;
        this.performanceArcState.lastTick = now;
        this.performanceArcState.lastNotifiedAt = now;
        this.performanceArcState.lastStageChangeAt = now;

        const nextStage = arc.stages[nextIndex];
        if (nextStage) {
            this.applyArcStage(arc, nextStage);
        }

        this.notifyPerformanceArcListeners();
    }

    applyArcStage(arc, stage) {
        if (!arc || !stage) {
            return null;
        }

        const snapshot = stage.profileSnapshot
            || (stage.storyboardEntryId && this.storyboard ? this.storyboard.getEntry(stage.storyboardEntryId)?.profileSnapshot : null);
        ...
        const profile = {
            id: stage.profileId || stage.storyboardEntryId || `arc-${arc.id}-${stage.id}`,
            prompt: stage.prompt || snapshot.mood?.description || arc.description || 'Performance arc stage',
            source: 'performance-arc',
            parameters: snapshot.parameters ? { ...snapshot.parameters } : {},
            palette: snapshot.palette ? { ...snapshot.palette } : null,
            mood: snapshot.mood ? { ...snapshot.mood } : null,
            metadata: {
                performanceArcId: arc.id,
                performanceArcLabel: arc.label,
                performanceStageId: stage.id,
                performanceStageLabel: stage.label,
                storyboardEntryId: stage.storyboardEntryId || null,
                loopCount: this.performanceArcState.loopCount
            }
        };

        return this.applyProfile(profile, {
            logType: 'performance-arc',
            logExtras: {
                arcId: arc.id,
                arcLabel: arc.label,
                stageId: stage.id,
